function loadRegion() {
  ...
  let currentRegionURL = 'https://script.google.com/macros/s/AKfycbz5irjd_bPlJ40DzxvmRX-8PC93Gc8h4BUoY9pybraN8R9bUVrIxEuBuq8134W5dJuVNA/exec'; //  with the actual URL
        ...
        switch (localStorage.getItem('role')) {
            ...
            case 'SL':
              ...
              <th>Audit Status</th>
                ...
                tdRange = Array.from({length: 30}, (_, i) => i);
              break;
            case 'admin':
              th = `
              <thead>
...
function displayDataForState(selectedState) {
  ...
  if (filteredData && filteredData.length > 0) {
      filteredData.forEach((row) => {
          ...
          const maxIndex = role === 'admin' ? 33 : 30; // AG if role is admin, AF otherwise
...
function filterTable() {
  ...
  function downloadTable() {
    ...
    const sheetName = 'Nayara_old'; // set sheet name
    let header, tdRange;
    ...
    switch (localStorage.getItem('role')) {
      ...
      case 'SL':
        ...
        'ALL Report Status', 'Your Name', 'Remark', 'Audit Status',]; // set header row
        tdRange = [...Array(30).keys()];
        break;
      case 'admin':
        header = ['Date', 'Ro Code', 'RO Name', 'State', 'Location', 'Total Tank', 'Online Tank', 'Offline Tank', 'Tank Offline Remark', 
        'Total DU', 'Online DU', 'Offline DU', 'DU Offline Remark', 'Motherboard Make',
