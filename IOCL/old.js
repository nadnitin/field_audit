function loadRegion() {
  ...
  let currentRegionURL = 'https://script.google.com/macros/s/AKfycbwbSYIkrxgipZEgWhlbdevdtEnyXrXm6pNvAGlirXtEe46omi3I3VxLnsFK4biJ2cgotw/exec'; // Replace 'YOUR_SERVER_URL_HERE' with the actual URL
        ...
        switch (localStorage.getItem('role')) {
          ...
          case 'SL':
            th = `
              ...
              tdRange = Array.from({length: 33}, (_, i) => i);
            break;
          case 'admin':
            th = `
              <thead>
...
function displayDataForState(selectedState) {
  ...
  if (filteredData && filteredData.length > 0) {
      filteredData.forEach((row) => {
          ...
          const maxIndex = role === 'admin' ? 35 : 32; // AG if role is admin, AF otherwise
  ...
  // Function to filter table based on selected state
  ...
  function downloadTable() {
    ...
    const sheetName = 'IOCL_old'; // set sheet name
    let header, tdRange;
    ...
    XLSX.writeFile(wb, 'IOCL_Old_Audit.xlsx'); // save workbook as .xlsx file
  }
